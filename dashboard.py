...
# This allows you to set the TV to refresh automatically via the URL
query_params = st.query_params
is_tv_url_mode = query_params.get("tvmode") in ["true", "1", "yes"]
...
with st.sidebar:
    ...
    # This toggle still works manually, but the URL param overrides it
...
def render_main_board():
    ...
    # Show status indicator if URL Mode is active
    if is_tv_url_mode:
    ...
    # Hide exit button if triggered via URL (permanent display)
...
# --- THE 2-SECOND ENGINE ---
# Set to 2 seconds as requested for Saturday production speed
@st.fragment(run_every=2)
def auto_refresh_loop():
    render_main_board()

# Start the loop if Toggle is ON OR the URL parameter is present
if st.session_state.get("tv_toggle", False) or is_tv_url_mode:
    auto_refresh_loop()
else:
    render_main_board()
