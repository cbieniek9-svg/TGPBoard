...
# -------------------------
# CONFIG & SIDEBAR CONTROLS (MOVED TO TOP)
# -------------------------
...
# Render sidebar immediately so it never disappears
...
def complete_task(task_id, user):
    with conn.session as s:
        s.execute(
            text("""
                UPDATE tasks 
                SET status='Closed', closed_by=:user, time_closed=CURRENT_TIMESTAMP
...
# RENDER LAYER
# -------------------------
def render(vm, current_user, is_tv=False):
    now = vm["now"]

    st.markdown(f"### TGP CENTRE STORE — {vm['today']} \n {now.strftime('%b %d %Y %I:%M %p')}")
    st.markdown(f"**Grocery:** {vm['grocery']}  |  **Frozen:** {vm['frozen']}  |  **Staff:** {vm['staff_count']}  |  **Tasks:** {vm['task_minutes']} mins  |  **Truck Day:** {'YES' if vm['is_truck_day'] else 'NO'}  |  **ETA:** {vm['eta']}")
    st.divider()
    ...
    for _, r in vm["tasks"].iterrows():
        ...
        if is_tv:
            ...
            if current not in options: options.append(current)

            c2.selectbox("Assign", options, index=options.index(current), key=key_name, on_change=assign_task, args=(r["task_id"], key_name), label_visibility="collapsed")
            c3.button("Done", key=f"done_{r['task_id']}", on_click=complete_task, args=(r["task_id"], current_user))
    ...
    for _, r in vm["oos"].iterrows():
        if is_tv:
            ...
            c2.button("CLR", key=f"clr_{r['oos_id']}", on_click=complete_oos, args=(r["oos_id"], current_user))
...
def tv_loop(user):
    data = load_raw()
    render(compute_vm(data), user, is_tv=True)

if tv_mode:
    tv_loop(active_user)
else:
    data = load_raw()
    render(compute_vm(data), active_user, is_tv=False)
